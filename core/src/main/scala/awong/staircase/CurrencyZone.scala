...
abstract class CurrencyZone {
  ...
  abstract class AbstractCurrency {
    ...
    def from(other: CurrencyZone#AbstractCurrency): Currency = {
      import scala.math._
      make(round(other.amount.toDouble * Converter.exchangeRate(other.designation)(this.designation) ))
    }
...
object Switzerland extends CurrencyZone {
  abstract class Franc extends AbstractCurrency {
    def designation = "CHF"
  }
  type Currency = Franc
  def make(centime: Long) = new Franc {
    val amount = centime
  }
...
object Converter {
  def exchangeRate(sourceDesignation: String)(targetDesignation: String) = {
    var map = Map(
    ...
    map(sourceDesignation)(targetDesignation)
  }
}
