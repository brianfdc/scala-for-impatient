...
package parsing {
   ...
   */
  object HorstmannArithmeticParser extends JavaTokenParsers {
    def expr: Parser[Int] = {
      (term ~ opt(( "+" | "-") ~ expr) ) ^^ {
        case t ~ None => t
        case t ~ Some("+" ~ e) => t + e
  ...
  object IntermediateExprParser extends RegexParsers {
    val number = "[0-9]+".r
  ...
  object CSVParser extends JavaTokenParsers {
    ...
    def parse(text: String) = parseAll(integers, text)
  }
